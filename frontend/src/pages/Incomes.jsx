      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-5 animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900">{income.source}</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-green-50 rounded-xl">
              <p className="text-xs text-green-500 font-medium">Amount</p>
            ...
            <div className="p-4 bg-indigo-50 rounded-xl">
              ...
              <p className="text-lg font-semibold text-gray-900 mt-1">{formatDate(income.date)}</p>
            </div>
            <div className="p-4 bg-purple-50 rounded-xl">
              <p className="text-xs text-purple-500 font-medium">Recurring</p>
              <p className="text-lg font-semibold text-gray-900 mt-1">{income.is_recurring ? "Yes" : "No"}</p>
            </div>
            <div className="p-4 bg-orange-50 rounded-xl">
              <p className="text-xs text-orange-500 font-medium">Source</p>
              <p className="text-lg font-semibold text-gray-900 mt-1 capitalize">{income.source}</p>
            </div>
          </div>
          {income.description && (
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Description</p>
              <p className="text-gray-900">{income.description}</p>
            </div>
          )}
          {income.notes && (
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Notes</p>
              <p className="text-gray-900">{income.notes}</p>
            </div>
          )}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onCancel}>
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">Delete Income</h3>
            <p className="text-gray-500 text-sm mb-2">Are you sure you want to delete this income?</p>
            <p className="font-medium text-gray-900">"{target.source}"</p>
            <p className="text-green-600 font-semibold mt-1">+{formatCurrency(target.amount)}</p>
          </div>
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{incomes.length} total income records</p>
        </div>
        <div className="flex items-center gap-2">
          <button
              onClick={() => setUndoTarget(true)}
            className="px-3 py-2 text-sm bg-orange-50 text-orange-700 rounded-lg hover:bg-orange-100 font-medium flex items-center gap-1.5"
          >
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sm:p-6 space-y-4 animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 ${formErrors.source ? "border-red-400 bg-red-50" : "border-gray-300"}`}
                  value={form.source}
              ...
              <div>
                ...
                <input
                  ...
                  min="0.01"
                  required
                  className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 ${formErrors.amount ? "border-red-400 bg-red-50" : "border-gray-300"}`}
                  value={form.amount}
              ...
              <div>
                ...
                <input
                  type="date"
                  className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 ${formErrors.date ? "border-red-400 bg-red-50" : "border-gray-300"}`}
                  value={form.date}
            ...
            <div className="flex justify-end gap-2 pt-2">
              {editing && (
                <button type="button" onClick={resetForm} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium">
                  Cancel
                </button>
              )}
      ...
      {loading ? (
        <div className="flex flex-col items-center py-16">
          ...
          <p className="text-gray-400 text-sm">Loading incomes...</p>
        </div>
      ) : incomes.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 py-16 text-center">
          <FiTrendingUp className="w-12 h-12 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-500 text-lg mb-1">No incomes yet</p>
          <p className="text-gray-400 text-sm mb-4">Add your first income record</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/80">
                  ...
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Recurring</th>
                  <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody>
                {paginatedIncomes.map((inc) => (
                  <tr key={inc.id} className="border-b border-gray-50 hover:bg-gray-50/70 transition-colors group">
                    ...
                    <td className="px-4 py-3.5 text-sm text-gray-500">{formatDate(inc.date)}</td>
                    <td className="px-4 py-3.5 text-sm text-gray-500 max-w-[200px] truncate">{inc.description || "—"}</td>
                    <td className="px-4 py-3.5 text-center">
                      {inc.is_recurring ? (
                        ...
                        <span className="text-xs text-gray-400">Once</span>
                      )}
                    </td>
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setDetailTarget(inc)} className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="View details">
                          <FiFileText className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleEdit(inc)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                          <FiEdit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => setDeleteTarget(inc)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
              ...
              <div key={inc.id} className="p-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-600 text-sm font-bold shrink-0 mt-0.5">
                      {inc.source?.charAt(0)?.toUpperCase() || "I"}
                    </div>
                    <div>
                      <p className="font-medium text-gray-900">{inc.source}</p>
                      {inc.description && <p className="text-xs text-gray-400 mt-0.5">{inc.description}</p>}
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-xs text-gray-500">{formatDate(inc.date)}</span>
                        {inc.is_recurring && (
                          <span className="text-xs text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                            <FiRepeat className="w-3 h-3" /> Recurring
                          </span>
                        )}
                      </div>
                ...
                <div className="flex gap-2 mt-3 pt-3 border-t border-gray-50">
                  <button onClick={() => setDetailTarget(inc)} className="flex-1 text-xs py-1.5 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 font-medium flex items-center justify-center gap-1">
                    <FiFileText className="w-3.5 h-3.5" /> View
                  </button>
                  <button onClick={() => handleEdit(inc)} className="flex-1 text-xs py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 font-medium flex items-center justify-center gap-1">
                    <FiEdit2 className="w-3.5 h-3.5" /> Edit
                  </button>
                  <button onClick={() => setDeleteTarget(inc)} className="flex-1 text-xs py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 font-medium flex items-center justify-center gap-1">
          ...
          {totalPages > 1 && (
            <div className="px-4 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3">
              <p className="text-sm text-gray-500">Page {page} of {totalPages} ({incomes.length} results)</p>
              <div className="flex items-center gap-1.5">
                ...
                {(() => {
                  ...
                  for (let i = start; i <= end; i++) {
                    pages.push(
                      <button key={i} onClick={() => setPage(i)} className={`min-w-[36px] h-9 rounded-lg text-sm font-medium transition-colors ${i === page ? "bg-green-600 text-white" : "text-gray-600 hover:bg-gray-100"}`}>{i}</button>
                    );
                  }
                  return pages;
                })()}
                <button onClick={() => setPage(Math.min(totalPages, page + 1))} disabled={page === totalPages} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><FiChevronRight className="w-4 h-4" /></button>
                <button onClick={() => setPage(totalPages)} disabled={page === totalPages} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed"><FiChevronsRight className="w-4 h-4" /></button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setUndoTarget(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">This will reverse your last income operation.</p>
            </div>
            <div className="flex gap-3 mt-6">
