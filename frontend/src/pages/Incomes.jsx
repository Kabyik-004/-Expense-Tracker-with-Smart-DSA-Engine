...
export default function Incomes() {
  ...
  const uniqueSources = useMemo(() => {
    ...
    return incomes.filter((inc) => {
      const s = inc.source?.toLowerCase().trim();
          ...
          <div className="p-4 border-b border-gray-100 dark:border-gray-700 space-y-3">
            <div className="flex flex-col sm:flex-row gap-2">
              <SearchBar
                ...
                accentColor="green"
                loading={loading}
                recentSearchesKey="incomes-search-history"
                suggestions={searchInput ? uniqueSources.filter((s) => s?.toLowerCase().includes(searchInput.toLowerCase())).map((s) => ({ text: s })) : []}
                suggestionsLabel="Sources"
              />
              <div className="flex gap-2">
                <button
